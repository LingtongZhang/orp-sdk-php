...
class HowToUseTheSdkTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetEnabledIds() {
		...
		// checking if files were written correctly
		$file_content = unserialize(file_get_contents('statistic_'. $today . '.txt', FILE_USE_INCLUDE_PATH));
		$this->assertEquals($file_content, array(431260, 52193));
		$file_content = file_get_contents('13554_items_'. $today . '.txt', FILE_USE_INCLUDE_PATH);
		$this->assertContains('130952812', $file_content);
		$file_content = unserialize(file_get_contents('error_'. $today . '.txt', FILE_USE_INCLUDE_PATH));
		$this->assertEquals($file_content, array('id'=>1114,'code'=>440,'timestamp'=>1376482070,'message'=>'empty response'));

		//deleting created files
		unlink('statistic_'. $today . '.txt');
		unlink('error_'. $today . '.txt');
		unlink('13554_items_'. $today . '.txt');
